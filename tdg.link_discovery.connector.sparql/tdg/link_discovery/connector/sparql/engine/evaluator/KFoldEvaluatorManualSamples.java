...
public class KFoldEvaluatorManualSamples extends AbstractEvaluator{
	...
	public KFoldEvaluatorManualSamples(IEnvironment environment, Integer kFoldSize, String workingFolderName) {
		...
		// retrieve positive and negative links
		Set<Tuple<String,String>> positiveSampleLinks = generateNegativeReferenceLinks();
		Set<Tuple<String,String>> negativeSampleLinks = generateNegativeReferenceLinks();
		// sparse positive and negative links into subDatasets
		sparsePositiveReferenceLinks(positiveSampleLinks);
		sparseNegativeReferenceLinks(negativeSampleLinks);
		// persist caches into file
		String outputDirectory = initFileSystem(workingFolderName);
		persistCaches(outputDirectory);
		loggingStart(kFoldSize, outputDirectory);
	}
	...
	private void loggingStart(Integer kfoldSize, String workingDirectory) {
	...
	public Set<Tuple<String,String>> generatePositiveReferenceLinks() {
		Set<Tuple<String,String>> positiveReferenceLinks = Sets.newHashSet();
		
		FrameworkUtils.readSameAsLinks(this.environment.getExamplesFile()).stream().forEach(link -> positiveReferenceLinks.add(transformLinkStrIntoTuple(link)));
		
		return positiveReferenceLinks;
	}
	
	private Set<Tuple<String,String>> generateNegativeReferenceLinks() {
		Set<Tuple<String,String>> negativeReferenceLinks = Sets.newHashSet();
				
		FrameworkUtils.readDifferentFromLinks(this.environment.getExamplesFile()).stream().forEach(link -> negativeReferenceLinks.add(transformLinkStrIntoTuple(link)));
	...
	private void sparsePositiveReferenceLinks(Set<Tuple<String, String>> positiveReferenceLinks) {
		...
		for(KFoldEvaluatorCache subDatasetIndex:this.subDatasets) {
			...
			while(subPositiveReferenceLinks.size()<referenceLinkPerFold && positiveReferenceLinks.size()>0) {
				...
				if(!subPositiveReferenceLinks.contains(link)) {// this avoids repeated links in the subdataset
	...
	private void sparseNegativeReferenceLinks(Set<Tuple<String, String>> negativeReferenceLinks) {
		Integer referenceLinkPerFold = Math.round(negativeReferenceLinks.size()/subDatasets.size());
		for(KFoldEvaluatorCache subDatasetIndex:this.subDatasets) {
			Set<Tuple<String,String>> subNegativeReferenceLinks = Sets.newHashSet();
			
			while(subNegativeReferenceLinks.size()<referenceLinkPerFold && negativeReferenceLinks.size()>0) {
				@SuppressWarnings("unchecked")
				Tuple<String,String> link = (Tuple<String, String>) Utils.getRandomElement(negativeReferenceLinks);
				if(!subNegativeReferenceLinks.contains(link)) {// this avoids repeated links in the subdataset
					negativeReferenceLinks.remove(link);
					subNegativeReferenceLinks.add(link);
				}
			}
			subDatasetIndex.setNegativeReferenceLinks(subNegativeReferenceLinks);
		}
	}
	...
	private Tuple<String,String> transformLinkStrIntoTuple(String linkStr) {
		String[] links = null;
		if(linkStr.contains("<http://www.w3.org/2002/07/owl#sameAs>")) {
			links = linkStr.split("<http://www.w3.org/2002/07/owl#sameAs>");
		}else if(linkStr.contains("owl:sameAs")) {
			links = linkStr.split("owl:sameAs");
		}else if(linkStr.contains("<http://www.w3.org/2002/07/owl#differentFrom>")) {
			links = linkStr.split("<http://www.w3.org/2002/07/owl#differentFrom>");
		}else if(linkStr.contains("owl:differentFrom")) {
			links = linkStr.split("owl:differentFrom");
		}else {
			System.out.println("Check this out, there is an error line 221 of kFoldEvaluatorManuealSamples");
		}
