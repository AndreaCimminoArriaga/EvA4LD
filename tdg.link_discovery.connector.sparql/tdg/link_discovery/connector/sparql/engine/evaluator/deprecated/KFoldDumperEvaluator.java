...
import tdg.link_discovery.connector.sparql.engine.evaluator.linker.SequentialLinker;
...
public class KFoldDumperEvaluator extends AbstractEvaluator{
	...
	public KFoldDumperEvaluator(IEnvironment environment, Integer kFoldSize, String workingFolderName) {
	...
	public KFoldDumperEvaluator(IEnvironment environment, Integer kFoldSize) {
		super(environment,null);
		this.environment = environment;
		this.kFoldSize = kFoldSize;
		this.currentIndexValidationFold= 0;
		...
		this.outputDirectory.append(FrameworkConfiguration.FRAMEWORK_WORKSPACE_DIRECTORY).append("/results/");
		this.outputDirectory.append(this.environment.getName()).append("/");
		this.outputDirectory.append(kFoldSize).append("-fold_datasets").append("/");
		// Init list of TDB datasets directory names	
		this.tdbFoldDatasetsDirectory = new CopyOnWriteArrayList<List<String>>();
		initTDBDatasetsDirectories();
	...
	public void foldDatasets(Boolean useNegatives, Boolean balancedReferenceLinks, Boolean useNumberOfNegativeLinks, Integer numberOfNegativeLinks) {
		...
		KFoldDumper kFoldDumper = new KFoldDumper(environment, kFoldSize, tdbFoldDatasetsDirectory, outputDirectory.toString());
	...
	private void linkSubDatasets(String sourceTDB, String targetTDB, Integer current_fold, Tuple<String,String> queries) {
		...
		linker.setDatasetSource(sourceTDB);
		linker.setDatasetTarget(targetTDB);
	...
	private Set<Tuple<String, String>> readReferenceLinks(String referenceLinksTDB, Boolean readPositives) {
		...
		String predicate = "http://www.w3.org/2002/07/owl#differentFrom";
		if(readPositives)
			predicate = "http://www.w3.org/2002/07/owl#sameAs";
		Dataset tdbDataset = TDBFactory.createDataset(referenceLinksTDB);
		tdbDataset.begin(ReadWrite.READ);
		String sparql = createQuery(predicate);
		Query query = QueryFactory.create(sparql);
		QueryExecution qexec = QueryExecutionFactory.create(query, tdbDataset);
		ResultSet results = qexec.execSelect();
		StreamUtils.asStream(results).forEach(rs -> 
			referenceLinks.add(new Tuple<String,String>(rs.getResource("?s").toString(), rs.getResource("?o").toString())));
		tdbDataset.end();
		return referenceLinks;
	}


	private String createQuery(String predicate) {
		StringBuffer query = new StringBuffer();
		query.append("SELECT DISTINCT ?s ?o WHERE { ?s <").append(predicate).append("> ?o . } ");
