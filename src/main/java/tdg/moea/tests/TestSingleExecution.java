...
public class TestSingleExecution {
	...
	public static void main(String[] args) throws Exception {
		...
		String algorithmFile  = "./experiments/algorithms/moea-carvalho.cnf";
		String environmentFile = "./experiments/environments/restaurantsZ.cnf";
		
		
		// Auxiliary input 
		String algorithmSetupFile = Paths.get(algorithmFile.replace(".cnf", "_setup.cnf")).toString();//"./experiments/algorithms/moea-eagle_setup.cnf";
		String agorithmDescription = Paths.get(algorithmFile).toString();;
		Integer execution = 1;
		Integer executionsNumber = Integer.valueOf(execution);
		Integer kFoldSize = 2;
		Integer maxSampleSize = 1600;
		...
		// Execute
		String workingFolderName = "/setup/"+kFoldSize+"-fold_datasets-"+Utils.getCurrentTime();
	...
	private static void executeLearner(String workingFolderName, IEnvironment environment, Integer kFoldSize, Setup setup, Map<String,String> description, Integer maxSampleSize) {
		Integer validationCounter =0;
		KFoldEvaluatorInMemory evaluator = new KFoldEvaluatorInMemory(environment, kFoldSize, workingFolderName);		
		evaluator.foldDatasets(true, true, false, -1, maxSampleSize);
		...
		while(validationCounter<=kFoldSize) {
			...
			if(learner.getAttributeSelector()!=null) {
				learner.getAttributeSelector().setModels(evaluator.getTrainingModels());
				learner.getAttributeSelector().setPositiveExamples(evaluator.getPositiveInstances());
			}
		...
		String foldedDataDirectory = Utils.getAbsoluteSystemPath(evaluator.getOutputDirectory().toString());
