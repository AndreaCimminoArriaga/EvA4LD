...
public class Test {
	...
	public static void main(String[] args) throws Exception {
		...
		KFoldEvaluator.DEFAULT_THREAD_POOL_SIZE = 2;	// given a kFoldSize, the number of referenceLinks that can be retrieved and stored in parallel
		LinkerKFoldCacheTDB.DEFAULT_THREAD_POOL_SIZE=60; // given a link rule, the number of instances that can be checked in parallel
		...
		GeneticProgrammingAlgorithm.DEFAULT_THREAD_POOL_SIZE_VARIATIONS = 60;
		
	
		String environmentFile = args[0].trim();
		String algorithm = args[1].trim();
		String setupNumberStr =args[2].trim();
		
		String folds = args[3].trim();
		String referenceLinksSize = args[4].trim();
		
		Integer executions = Integer.valueOf(args[5].trim())+1;
		
		int startingSetup = 0;
		int finalSetup = 0;
		int initialSetup = 0;
		if(setupNumberStr.equals("0")) {
			startingSetup = 0;
			finalSetup = 16;
		}else if(setupNumberStr.equals("1")) {
			startingSetup = 16;
			finalSetup = 29;
		}else if(setupNumberStr.equals("2")) {
			startingSetup = 29;
			finalSetup = 39;
		}else if(setupNumberStr.equals("3")) {
			startingSetup = 39;
			finalSetup = 49;
		}
		initialSetup = startingSetup;
		
		for(int i=1;i<executions;i++) {
			Integer execution = i;
	
			while( startingSetup <finalSetup) {
				Integer setupNumber = startingSetup;//Integer.valueOf(setupNumberStr);
				System.out.println("execution: "+execution+" setup: "+setupNumber);
				runOverEnvironment(environmentFile, algorithm, folds, execution.toString(), setupNumber, referenceLinksSize);
				System.gc();
				startingSetup++;
			}
			startingSetup = initialSetup;
		}
		
		/*
		for(int i=1; i<246;i++) {
			Integer index = i-1;
			String execution = String.valueOf((index/49)+1);
			Integer setupNumber = (index%49);	
			//System.out.println("Executing with environment: "+environmentFile);
			System.out.println("execution: "+execution+" setup: "+setupNumber);
			runOverEnvironment(environmentFile, algorithm, folds, execution, setupNumber, referenceLinksSize);
			break;
		}
		*/
	...
	private static void runOverEnvironment(String envorinmentDirectoy, String algorithmDirectory, String kFoldSizeStr, String execution, Integer index, String referenceLinksSize) throws Exception {
		...
		Integer maxSampleSize = Integer.valueOf(referenceLinksSize);
		...
		executeLearner(workingFolderName, environment, kFoldSize, setup, description, maxSampleSize);
	...
	private static void executeLearner(String workingFolderName, IEnvironment environment, Integer kFoldSize, Setup setup, Map<String,String> description, Integer referenceLinksSize) {
		Integer validationCounter =0;
		KFoldEvaluator evaluator = new KFoldEvaluator(environment, kFoldSize, workingFolderName,referenceLinksSize);		
